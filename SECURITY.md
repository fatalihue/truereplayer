...
- **Macros fazem o que você mandou.** Um perfil pode digitar qualquer coisa, inclusive uma senha que você mesmo colocou nele — perfis são arquivos de texto sem criptografia. Trate um `.trprofile` recebido de terceiros como você trataria um script: leia antes de rodar. Na primeira importação o app mostra um aviso geral do que um perfil importado consegue fazer — ele não inspeciona o arquivo, e pode ser dispensado de vez. O que o código garante é outra coisa, e mais forte: uma automação importada chega **desarmada** e não dispara até você armá-la.
...
**Out of scope.** That the app simulates input and reads the screen — that is the product. That a macro does what it was told, including typing a password someone put in it: profiles are plain text, so treat a `.trprofile` from a stranger like any other script and read it first. The importer shows a general warning on your first import — it does not inspect the file, and it can be dismissed for good; what the code guarantees instead, and it is the stronger property, is that an imported automation arrives **disarmed** and cannot fire until you arm it. The extension manifest `key` is an RSA *public* key that pins the extension ID. Scanner output with no demonstrated impact.
